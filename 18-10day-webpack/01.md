...
> 图里边的3000端口，是我配置了port为3000的缘故。以上之所以有响应内容出现是因为，我们是有build目录。如果没有build目录，那么你就得配置html插件生成一个index.html到内存里边的build目录里边去，不然，会报cannot……这样的错误
...
首先你得知道，webpack本身就是用各种插件堆叠起来的，而关于插件的用法是简单：

1. 安装插件
2. require插件
3. 在plugins这个数组里边 push 一个 「new 插件」（插件是个类哈），如 `new HtmlWebpackPlugin({})`
4. 传入的 `{}` 即是用来配置选项参数的，通过配置这些参数，new出来的实例，才能更好地运作起来
